...
public GalleryCreateResponseDTO write(Long memberId, GalleryCreateRequestDTO requestDTO, MultipartFile coverFile) {
    ...
    requestDTO.setCoverImage(saveCoverImage(coverFile));     //  커버 이미지를 저장 /  업로드된 파일을 S3 object key로 바꿔서 DB에는 경로만 저장했습니다.
    ...
    saveTags(requestDTO.getId(), requestDTO.getTagIds(), requestDTO.getTagNames()); // 발표용: 태그를 연결 /  tag association을 정규화해서 저장했습니다.
    galleryDAO.updateWorkCount(requestDTO.getId());          // 작품 수를 갱신 /  화면 집계를 위해 denormalized count column을 업데이트했습니다.
...
public WorkCreateResponseDTO write(Long memberId, WorkCreateRequestDTO requestDTO,
    ...
    saveTags(workDTO.getId(), requestDTO.getTagIds(), requestDTO.getTagNames()); //  태그를 연결 / 개발자용: 태그는 join table로 정규화해서 저장했습니다.
    saveGalleryLink(galleryId, workDTO.getId());             //  예술관에 연결 /  parent-child 관계를 별도 매핑 테이블로 연결했습니다.
...
public BidResponseDTO placeBid(Long memberId, BidRequestDTO requestDTO) {
    ...
    bidDAO.clearPreviousWinning(requestDTO.getAuctionId());  //  이전 최고 입찰을 해제합니다 /  기존 winning bid 플래그를 먼저 false로 바꿨습니다.
...
public PaymentResponseDTO confirmBootpayPayment(Long buyerId, BootpayConfirmRequestDTO requestDTO) {
    ...
    if (!payment.getBuyerId().equals(buyerId)) throw new IllegalArgumentException("결제 검증 권한이 없습니다."); //  결제 권한을 확인합니다 /  buyerId 비교로 ownership check를 수행했습니다.
    ...
    return completePayment(payment.getId(), buyerId);        //  결제를 완료합니다 
```
...
public GalleryDetailResponseDTO getGalleryDetail(Long id) {
    ...
    detail.setIsLiked(memberId != null && galleryDAO.existsLike(memberId, id)); //  좋아요 상태를 보여줍니다 /  현재 사용자 기준 reaction state를 계산했습니다.
    detail.setIsBookmarked(memberId != null && bookmarkDAO.exists(memberId, "GALLERY", id)); // 북마크 상태를 보여줍니다 /  bookmark 여부를 projection으로 주입했습니다.
...
public WorkDetailResponseDTO getWorkDetail(Long id) {
    ...
    applyFileUrls(detail);                                   //  파일 경로를 바꿉니다 / 개발자용: media asset key를 response DTO에서 쓸 URL로 변환했습니다.
    ...
    detail.setIsOwner(memberId != null && memberId.equals(detail.getMemberId())); // 소유자 여부를 보여줍니다 /  owner check를 통해 현재 사용자 소유 여부를 계산했습니다.
...
public DashboardResponseDTO getDashboard(Long memberId) {
    ...
    analyticsMetrics.put("works", buildMetricSeries("works", "내가 만든 작품 추이",
            ...
            true, "내 작품", "개", endDate)); //  작품 추이 그래프를 만듭니다
    analyticsMetrics.put("galleries", buildMetricSeries("galleries", "내가 만든 예술관 추이",
            ...
            true, "내 예술관", "개", endDate)); //예술관 추이 그래프를 만듭니다 / 작품과 예술관 series를 분리해서 집계했습니다.

    return DashboardResponseDTO.builder()
            .totalViewsText(formatCompactNumber(nullSafe(summary.getTotalViews()))) //  조회수를 보기 좋게 보여줍니다 /  숫자를 compact format으로 바꿔 KPI 텍스트로 사용했습니다.
            .activeAuctionsText(formatTwoDigits(nullSafe(summary.getActiveAuctionCount()))) //  경매 수를 보여줍니다 / 운영 지표를 두 자리 형태로 정규화했습니다.
            .pendingPaymentsText(formatCurrencyCompact(nullSafe(summary.getPendingPaymentAmount()))) //  결제 금액을 요약해 보여줍니다 /  금액을 compact currency format으로 변환했습니다.
            .analyticsMetrics(analyticsMetrics)
            .ownedWorks(withFallback(dashboardMapper.selectOwnedWorkItems(memberId), "등록된 작품이 없습니다.", "")) //  작품이 없으면 안내 문구를 보여줍니다 / empty list에 fallback 메시지를 넣었습니다.
            .galleries(withFallback(dashboardMapper.selectGalleryItems(memberId), "예술관이 없습니다.", "")) //  예술관이 없으면 안내 문구를 보여줍니다 /  collection empty-state를 처리했습니다.
            .paymentHistory(withFallback(dashboardMapper.selectPaymentHistoryItems(memberId), "결제 이력이 없습니다.", "")) //  결제 내역이 없으면 안내 문구를 보여줍니다 /  history list에 fallback을 적용했습니다.
            .build();
}
```
